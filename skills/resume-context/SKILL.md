...
5. If `state/lessonbook/inbox.md` (`$LESSONBOOK_INBOX`) has `UNREVIEWED` placeholders left by a `PreCompact` hook, surface them — they may hold a correction from before a compaction. Hand them to `correction-capture` to triage.
6. Any unmet precondition or open question → record it in `state/tasks/<task_id>/run_state.yaml` (do not silently proceed past it).
